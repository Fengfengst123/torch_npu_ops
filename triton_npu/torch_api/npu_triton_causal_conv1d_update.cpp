...
torch::Tensor npu_causal_conv1d_update(
    ...
    if (!cache_seqlens.has_value() && !num_accepted_tokens.has_value()) {
        gridX = batch;
        auto& op = OperationFactory::instance().causal_conv1d_update_no_cache_no_mtp();
        ...
        if (ret != RT_ERROR_NONE) {
            LOG(ERROR) << "rtKernelLaunch failed for '_causal_conv1d_update_kernel_no_cache_len_no_mtp': " << ret;
            return out;
        }
    } else {
        LOG(ERROR) << "currently causal_conv1d_update don't support input cache_len and mtp scenerios.";
    }
    return out;
}
