...
#ifndef XLLM_LAYER_NORM_FWD_SINGLE_READ_Z_H
#define XLLM_LAYER_NORM_FWD_SINGLE_READ_Z_H
...
class LayerNormFwdSingleReadZ {
public:
    __aicore__ inline LayerNormFwdSingleReadZ() {}
        ...
        const optiling::LayerNormFwdTilingData* __restrict tilingData)
    {
        ...
        normBeforeGate = tilingData->normBeforeGate;
        ...
        zGm.SetGlobalBuffer((__gm__ Tfm*)z, colSize * rowSize);
...
private:
    ...
    __aicore__ inline void LoadGate(LocalTensor<float> gateLocal, uint32_t currentLogicalRow, uint32_t currentNRow)
    {
        DataCopyExtParams dataCopyParams;
        DataCopyPadExtParams<Tfm> padParams;
        dataCopyParams.blockCount = currentNRow;
        ...
        DataCopyPad(
            gateLocal.ReinterpretCast<Tfm>()[(sizeof(Tfm) == kHalfBytes) * tileLength],
            zGm[currentLogicalRow * rowSize],
            dataCopyParams,
            padParams);
        SetEvtFlag<HardEvent::MTE2_V>();
        if (sizeof(Tfm) == kHalfBytes) {
            Cast(gateLocal, gateLocal.ReinterpretCast<Tfm>()[tileLength], RoundMode::CAST_NONE, tileLength);
            PipeBarrier<PIPE_V>();
        }
    ...
    __aicore__ inline void LoadWeight(LocalTensor<float> dst, GlobalTensor<Tweight> src, uint32_t groupOffset)
    {
        DataCopyExtParams copyParams;
    ...
    __aicore__ inline void ProcessBasicBlock(uint32_t currentNRow, uint32_t currentLogicalRow)
    {
        ...
        LocalTensor<float> gateLocal = zBuf.Get<float>();

        LoadInput(xLocal, currentLogicalRow, currentNRow);
        LoadGate(gateLocal, currentLogicalRow, currentNRow);
        if (!normBeforeGate) {
            Mul(xLocal, xLocal, gateLocal, tileLength);
            PipeBarrier<PIPE_V>();
        }
        ...
        if (normBeforeGate) {
            Mul(yLocal, yLocal, gateLocal, tileLength);
            PipeBarrier<PIPE_V>();
        }
...
#endif // XLLM_LAYER_NORM_FWD_SINGLE_READ_Z_H
