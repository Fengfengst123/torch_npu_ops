/*
 * Generated AscendC kernel for xLLM layer_norm_fwd.
 *
 * This file is intentionally kept independent from the PyTorch extension
 * wrapper. Build it as a custom OPP kernel that exports aclnnXllmLayerNormFwd.
 */
...
__aicore__ inline void Silu(LocalTensor<float> dst,
  ...
  PipeBarrier<PIPE_V>();
  Reciprocal(backup, backup, count);
  PipeBarrier<PIPE_V>();
  Mul(dst, src, backup, count);
  PipeBarrier<PIPE_V>();
}
...
__aicore__ inline float RstdFromVariance(LocalTensor<float> scalar,
  ...
  if constexpr (sizeof(T) == sizeof(float)) {
    ...
    return RsqrtValue(scalar, variance + eps);
  }
}
...
class KernelFullRow {
 ...
 private:
  ...
  __aicore__ inline void ProcessRmsGateN128() {
    const uint32_t base_rows = m_ / blockDim_;
    const uint32_t extra_rows = m_ - base_rows * blockDim_;
    uint32_t row_start = 0;
    uint32_t rows_left = base_rows;
    if (coreId_ < extra_rows) {
      rows_left += 1;
      row_start = coreId_ * rows_left;
    } else {
      row_start = extra_rows * (base_rows + 1) +
                  (coreId_ - extra_rows) * base_rows;
    }
    if (rows_left == 0) {
      return;
    }
    ...
    LoadWeightN128();
    while (rows_left > 0) {
      const uint32_t rows = MinU32(tileRows_, rows_left);
      ProcessTileRmsGateN128(row_start, rows);
      row_start += rows;
      rows_left -= rows;
    }
  }
  ...
  __aicore__ inline void ProcessRmsN128NoZ() {
    ...
    while (rows_left > 0) {
      const uint32_t rows = MinU32(tileRows_, rows_left);
      ProcessTileRmsN128NoZ(row_start, rows);
      row_start += rows;
      rows_left -= rows;
    }
  }
  ...
  __aicore__ inline void ProcessTileRmsGateN128(uint32_t row_start,
    ...
    DataCopyExtParams rstd_params;
    rstd_params.blockCount = 1;
    rstd_params.blockLen = rows * sizeof(float);
    rstd_params.srcStride = 0;
    rstd_params.dstStride = 0;
    WaitSToMte3();
    DataCopyPad(rstdGm_[row_start], rstd_stat, rstd_params);
  ...
  __aicore__ inline void ProcessLinearRows() {
    const uint32_t total = m_ * ngroups_;
    for (uint32_t linear = coreId_; linear < total; linear += blockDim_) {
      const uint32_t group = linear / m_;
      const uint32_t row = linear - group * m_;
      ProcessTile(group, row, 1);
    }
  }
  ...
  __aicore__ inline void ProcessRowBlocks() {
    for (uint32_t group = 0; group < ngroups_; ++group) {
      ...
      while (rows_left > 0) {
        const uint32_t rows = MinU32(tileRows_, rows_left);
        ProcessTile(group, row_start, rows, true);
        row_start += rows;
        rows_left -= rows;
      }
    }
  ...
  __aicore__ inline void ProcessTile(uint32_t group,
    ...
    if (isRmsNorm_ == 0) {
      DataCopyExtParams stat_params;
      stat_params.blockCount = 1;
      stat_params.blockLen = rows * sizeof(float);
      stat_params.srcStride = 0;
      stat_params.dstStride = 0;
      WaitSToMte3();
      DataCopyPad(meanGm_[group * m_ + row_start], mean_stat, stat_params);
    }
    DataCopyExtParams rstd_params;
    rstd_params.blockCount = 1;
    rstd_params.blockLen = rows * sizeof(float);
    ...
    rstd_params.dstStride = 0;
    WaitSToMte3();
    DataCopyPad(rstdGm_[group * m_ + row_start], rstd_stat, rstd_params);
...
class KernelStreamingTwoPass {
 public:
  ...
  __aicore__ inline void Init(GM_ADDR x,
    ...
    pipe_.InitBuffer(scalarBuf_, kFp32BlockElems * sizeof(float));
    pipe_.InitBuffer(weightRawBuf_, chunkAlign_ * sizeof(TW));
    pipe_.InitBuffer(weightFp32Buf_, chunkAlign_ * sizeof(float));
    if (hasBias_ != 0) {
      pipe_.InitBuffer(biasRawBuf_, chunkAlign_ * sizeof(TW));
 ...
 private:
  ...
  __aicore__ inline void LoadParamChunk(uint32_t group,
                                        uint32_t col,
                                        uint32_t valid) {
    const uint32_t right_pad = chunkAlign_ - valid;
    LocalTensor<TW> w_raw = weightRawBuf_.Get<TW>();
  ...
  __aicore__ inline void ProcessLogicalRow(uint32_t group, uint32_t row) {
    float sum_val = 0.0f;
    ...
    for (uint32_t col = 0; col < groupSize_; col += chunkSize_) {
      const uint32_t valid = MinU32(chunkSize_, groupSize_ - col);
      LocalTensor<T> x_local = xQueue_.AllocTensor<T>();
      ...
      if (hasZ_ != 0 && normBeforeGate_ == 0) {
        ...
        PipeBarrier<PIPE_V>();
        Silu(tmp_fp32, tmp_fp32, reduce_tmp, valid);
        Mul(x_fp32, x_fp32, tmp_fp32, valid);
        PipeBarrier<PIPE_V>();
        zQueue_.FreeTensor(z_local);
      }
      ...
      if (isRmsNorm_ == 0) {
        sum_val += ReduceSumValue(scalar, x_fp32, tmp_fp32, reduce_tmp, valid);
      }
      Mul(tmp_fp32, x_fp32, x_fp32, valid);
      PipeBarrier<PIPE_V>();
      square_sum_val += ReduceSumValueNoCopy(scalar, tmp_fp32, reduce_tmp, valid);
      xQueue_.FreeTensor(x_local);
    }
    ...
    if (isRmsNorm_ == 0) {
      ...
      LocalTensor<float> mean_one = scalar;
      mean_one.SetValue(0, mean_val);
      DataCopyExtParams stat_params;
      stat_params.blockCount = 1;
      stat_params.blockLen = sizeof(float);
      stat_params.srcStride = 0;
      stat_params.dstStride = 0;
      WaitSToMte3();
      DataCopyPad(meanGm_[group * m_ + row], mean_one, stat_params);
    }
    const float rstd_val = RstdFromVariance<T>(scalar, variance, eps_);
    scalar.SetValue(0, rstd_val);
    DataCopyExtParams rstd_params;
    rstd_params.blockCount = 1;
    rstd_params.blockLen = sizeof(float);
    rstd_params.srcStride = 0;
    rstd_params.dstStride = 0;
    WaitSToMte3();
    DataCopyPad(rstdGm_[group * m_ + row], scalar, rstd_params);
    ...
    for (uint32_t col = 0; col < groupSize_; col += chunkSize_) {
      ...
      if (isRmsNorm_ == 0) {
        Adds(x_fp32, x_fp32, -mean_val, valid);
        PipeBarrier<PIPE_V>();
      }
      Muls(x_fp32, x_fp32, rstd_val, valid);
      PipeBarrier<PIPE_V>();
      LoadParamChunk(group, col, valid);
      Mul(x_fp32, x_fp32, weightFp32Buf_.Get<float>(), valid);
      PipeBarrier<PIPE_V>();
      if (hasBias_ != 0) {
        Add(x_fp32, x_fp32, biasFp32Buf_.Get<float>(), valid);
        PipeBarrier<PIPE_V>();
      }
      if (hasZ_ != 0 && normBeforeGate_ != 0) {
        CastToFloat<T>(tmp_fp32, z_local, chunkAlign_);
      ...
      y_local = yQueue_.DeQue<T>();
      WaitVToMte3();
      CopyOutPad<T>(yGm_[row * strideY_ + group * groupSize_ + col],
                    y_local,
                    1,
                    valid * sizeof(T),
                    0,
...
extern "C" __global__ __aicore__ void xllm_layer_norm_fwd(
    GM_ADDR x,
    GM_ADDR weight,
    GM_ADDR bias,
    GM_ADDR z,
  ...
  if (kernel_mode == xllm_layer_norm_fwd::kStreamingTwoPass) {
    xllm_layer_norm_fwd::KernelStreamingTwoPass<DTYPE_X, DTYPE_WEIGHT> op;
    op.Init(x,
            weight,
            bias,
            z,
  ...
  xllm_layer_norm_fwd::KernelFullRow<DTYPE_X, DTYPE_WEIGHT> op;
  op.Init(x,
          weight,
          bias,
          z,
